...
# Cấu hình thời gian lưu trữ chat history (1 giờ)
CHAT_HISTORY_RETENTION_HOURS = 1


def cleanup_old_chat_history(db: Session):
    """
    Xóa tất cả lịch sử chat cũ hơn 1 giờ
    """
    try:
...
def get_session_chat_history(session_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Lấy lịch sử chat của một session cụ thể trong vòng 1 giờ gần đây"""
